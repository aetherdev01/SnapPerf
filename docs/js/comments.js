...
function spApplyUserToBar(bar){
  ...
  var inp=sec?sec.querySelector('.sp-cmt-input'):null;
  if(inp&&u)inp.placeholder='Tulis komentar...';
}
  ...
  try{
    var ch=spSb.channel('cmt_'+tag)
      .on('postgres_changes',{event:'INSERT',schema:'public',table:'sp_comments',filter:'release_tag=eq.'+tag},function(payload){
        var d=payload.new;
        if(!d||!d.text)return;
        var existing=list?list.querySelector('.sp-cmt-optimistic'):null;
        var uid=spUser?spUser.uid:spGetAnonId();
        if(existing&&d.user_id===uid&&d.text===existing.querySelector('.sp-cmt-text').textContent){
          existing.classList.remove('sp-cmt-optimistic');
          existing.style.opacity='1';
          var te=existing.querySelector('.sp-cmt-time');
          if(te){te.textContent=spRelTime(d.created_at);te.dataset.ts=d.created_at;te.title=spFullDate(d.created_at);}
          return;
        }
        if(list){
          var el=spMakeCmtEl(d);
          el.classList.add('sp-cmt-realtime');
          list.appendChild(el);
...
async function spSendComment(tag,text,isAnon,section,bar){
  ...
  var optEl=spMakeCmtEl(optData,true);
  ...
  try{
    ...
    if(saved&&saved.created_at){
      ...
      if(te){te.textContent=spRelTime(saved.created_at);te.dataset.ts=saved.created_at;te.title=spFullDate(saved.created_at);}
    }
    optEl.style.opacity='1';optEl.classList.remove('sp-cmt-optimistic');
    spShowToast('\u2713 Komentar terkirim!',true);
    ...
    if(errMsg.indexOf('relation')!==-1||errMsg.indexOf('does not exist')!==-1){
      spShowToast('\u2717 Tabel belum dibuat. Jalankan SUPABASE_SETUP.sql',false);
    }else if(errMsg.indexOf('policy')!==-1||errMsg.indexOf('violates')!==-1){
      spShowToast('\u2717 RLS policy error. Cek Supabase.',false);
    }else{
      spShowToast('\u2717 Gagal kirim: '+(errMsg.slice(0,60)||'error'),false);
    }
...
async function spToggleLike(tag,bar){
  ...
  try{
    ...
    if(ex.data&&ex.data.length>0){
      var d=await spSb.from('sp_likes').delete().eq('release_tag',tag).eq('user_id',uid);
      if(d.error)throw d.error;
    }else{
      var i=await spSb.from('sp_likes').insert({release_tag:tag,user_id:uid});
...
function spBuildEngage(tag){
  ...
  return '<div class="sp-engage-bar-wrap">'
    ...
    +'<input class="sp-cmt-input" type="text" placeholder="'+(u?'Tulis komentar...':'Tulis komentar...')+'" maxlength="280" autocomplete="off" spellcheck="false"/>'
