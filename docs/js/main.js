...
function applyLang(){var lang=getCurLang();document.querySelectorAll('[data-i18n]').forEach(function(el){var v=t(el.getAttribute('data-i18n'));if(v!=null)el.textContent=v;});document.querySelectorAll('[data-i18n-placeholder]').forEach(function(el){var v=t(el.getAttribute('data-i18n-placeholder'));if(v)el.placeholder=v;});var lbl=document.getElementById('langLabel');if(lbl)lbl.textContent=lang==='id'?'ID':'EN';document.querySelectorAll('.mob-lang-label').forEach(function(m){m.textContent=lang==='id'?'Switch to English':'Beralih ke Bahasa Indonesia';});document.documentElement.lang=lang==='id'?'id':'en';
  var sbLbl=document.getElementById('sbLangLabel');if(sbLbl)sbLbl.textContent=lang.toUpperCase();}
...
document.addEventListener('DOMContentLoaded',function(){
  ...
  function toggleLang(){var next=getCurLang()==='en'?'id':'en';localStorage.setItem('sp-lang',next);applyLang();document.body.style.transition='opacity .1s ease';document.body.style.opacity='0.65';requestAnimationFrame(function(){requestAnimationFrame(function(){document.body.style.opacity='1';setTimeout(function(){document.body.style.transition='';document.body.style.opacity='';},140);});});}
...
async function loadNotification(){
  var NOTIF_URL='https://aetherdev01.github.io/SnapPerf/server/notification.json';
  var FALLBACK_URL='https://raw.githubusercontent.com/aetherdev01/SnapPerf/main/server/notification.json';
  var d=null;
  try{var r1=await fetch(NOTIF_URL+'?t='+Date.now(),{cache:'no-store'});if(r1.ok)d=await r1.json();}catch(e){}
  if(!d){try{var r2=await fetch(FALLBACK_URL+'?t='+Date.now(),{cache:'no-store'});if(r2.ok)d=await r2.json();}catch(e){}}
  try{
