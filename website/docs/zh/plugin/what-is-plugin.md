...
插件概念建立在与 root 模块相同的架构上。它不是作为替代品，而是作为 **SnapPerf** 中的一个**附加功能**，引入了非 Root 版本。
...
SnapPerf 的插件支持显示界面和与用户交互，与 **KernelSU** 相同，但不需要 root 访问权限。更多详情请参阅 [KernelSU WebUI 文档](https://kernelsu.org/guide/module-webui.html)。

::: info 与 KernelSU 的区别
SnapPerf 支持 **KernelSU** API，仅在权限和如何处理 `webroot/index.html` 的本地目录方面有所不同
:::

## BusyBox

SnapPerf 附带一个功能完整的 BusyBox 二进制文件（包括完整的 SELinux 支持）。可执行文件位于 `/data/user_de/0/com.android.shell/axeron/bin/busybox`。

SnapPerf 的 BusyBox 支持运行时可切换的"ASH 独立 Shell 模式"。这种独立模式意味着当在 BusyBox 的 ash shell 中运行时，每个命令都将直接使用 BusyBox 内部的小程序，而不管 `PATH` 设置为什么。例如，`ls`、`rm`、`chmod` 等命令将**不会**使用 PATH 中的内容（在 Android 的情况下，默认将是 `/system/bin/ls`、`/system/bin/rm` 和 `/system/bin/chmod`），而是直接调用内部 BusyBox 小程序。这确保脚本始终在可预测的环境中运行，并且无论运行在哪个 Android 版本上，都具有完整的命令套件。要强制命令不使用 BusyBox，您必须使用完整路径调用可执行文件。

在 SnapPerf 上下文中运行的每个 shell 脚本都将在启用独立模式的 BusyBox `ash` shell 中执行。对于第三方开发者相关的内容，这包括所有启动脚本和模块安装脚本。

::: info 与 Magisk / KernelSU 的区别
SnapPerf 的 BusyBox 现在使用直接从 Magisk 项目编译的二进制文件。感谢 Magisk！因此，您不必担心 Magisk、KernelSU 和 SnapPerf 之间 BusyBox 脚本的兼容性问题，因为它们完全相同！
:::

## SnapPerf 插件

SnapPerf 插件是放置在 `/data/user_de/0/com.android.shell/axeron/plugins` 中的文件夹，结构如下：
...
│   ├── uninstall.sh        <--- 当 SnapPerf 移除您的模块时将执行此脚本
│   ├── action.sh           <--- 当用户在 SnapPerf 应用中点击操作按钮时将执行此脚本
...
SnapPerf 使用 **BOOT_COMPLETED** 接收器和 **热重启系统** 方法来启动 `post-fs-data.sh`、`system.prop`、`late_start service.sh`，所有其他文件在 root 模式下的处理方式相同。除了 root init 方法。
:::

### module.prop

module.prop 是模块的配置文件。在 SnapPerf 中，如果模块不包含此文件，它将不会被识别为模块。此文件的格式如下：
...
author=<string>
description=<string>
axeronPlugin=<int> <--- 用于定位支持的 SnapPerf 服务器版本的字段
```
...
::: warning
确保您使用的 `axeronPlugin=version` **等于或低于**您的 SnapPerf **服务器版本**。如果**服务器版本**低于 `axeronPlugin=version`，您将**无法**在 SnapPerf 上刷入它。
:::
...
::: warning 与 Root 方法的区别
您可以使用环境变量 `AXERON` 来确定脚本是在 SnapPerf、KernelSU 还是 Magisk 中运行。如果在 SnapPerf 中运行，此值将设置为 true。
:::
...
SnapPerf **目前**仅通过将 `/system/bin` 添加到 `PATH` 来影响它，因此那里的二进制文件可以在任何脚本中直接调用。
:::

## 插件安装程序

SnapPerf 插件安装程序是打包在 ZIP 文件中的 `root` 模块，可以在 SnapPerf 中刷入。最简单的 SnapPerf 插件安装程序只是打包为 ZIP 文件的 `root` 模块。
...
SnapPerf 插件严格遵循所使用的底层 `root` 模块的行为。
:::
...
customize.sh 脚本在启用独立模式的 SnapPerf BusyBox ash shell 中运行。以下变量和函数可用：

#### 变量

- `AXERON` (bool)：一个标记脚本在 SnapPerf 环境中运行的变量，此变量的值将始终为 true。您可以使用它来区分 SnapPerf、KernelSU 和 Magisk。
- `AXERONVER` (int)：当前运行的 SnapPerf 服务器的版本整数（例如 10400）。
- `BOOTMODE` (bool)：在 SnapPerf 中始终为 true。
